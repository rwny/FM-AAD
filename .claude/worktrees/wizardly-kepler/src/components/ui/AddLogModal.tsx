...
export const AddLogModal: React.FC<AddLogModalProps> = ({
  ...
  const [maintenanceType, setMaintenanceType] = useState<'PM' | 'CM' | 'IR' | ''>('')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const issueWithType = maintenanceType ? `[${maintenanceType}] ${issue}` : issue;
      if (category === 'AC') {
        ...
        const { error } = await supabase
          ...
          .insert({
            ...
            note: note || null,
            status,
            next_service: nextServiceDate || null
          })
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[16px] w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Next Service Date (Optional)</label>
            <input
              type="date"
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Maintenance Type (Optional)</label>
            <div className="grid grid-cols-4 gap-2">
              {([
                { value: '', label: 'General' },
                { value: 'PM', label: 'PM', desc: 'Preventive' },
                { value: 'CM', label: 'CM', desc: 'Corrective' },
                { value: 'IR', label: 'IR', desc: 'Inspection' },
              ] as const).map((t) => (
                <button
                  key={t.value}
                  type="button"
                  onClick={() => setMaintenanceType(t.value as typeof maintenanceType)}
                  className={`px-1 py-2 rounded-[8px] text-[10px] font-black uppercase transition-all border ${
                    maintenanceType === t.value
                      ? t.value === 'PM' ? 'bg-blue-500 text-white border-blue-600 shadow-md'
                        : t.value === 'CM' ? 'bg-orange-500 text-white border-orange-600 shadow-md'
                        : t.value === 'IR' ? 'bg-purple-500 text-white border-purple-600 shadow-md'
                        : 'bg-slate-600 text-white border-slate-700 shadow-md'
                      : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'
                  }`}
                  title={'desc' in t ? t.desc : 'No type'}
                >
                  {t.label}
                </button>
              ))}
