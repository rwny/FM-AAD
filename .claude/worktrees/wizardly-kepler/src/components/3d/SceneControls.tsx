...
export function CameraOffset({ leftVisible, rightVisible, sidebarWidth = 300 }: { leftVisible: boolean, rightVisible: boolean, sidebarWidth?: number }) {
  ...
  useFrame(() => {
    if (camera instanceof THREE.PerspectiveCamera) {
      ...
      // positive x in setViewOffset → camera sees content shifted right → model appears LEFT
      // right sidebar visible (r=280, l=0) → +140 → model shifts left to center in remaining space
