...
import { FurnitureRightPanel } from './components/modes/FurnitureMode'
import { ACRightPanel } from './components/modes/ACMode'
import { EERightPanel } from './components/modes/EEMode'
...
function App() {
  ...
  const [addLogAsset, setAddLogAsset] = useState<ACAsset | null>(null)
    ...
    async function loadData() {
      try {
        import('./utils/supabase').then(async ({ fetchBuildingData, fetchAllACLogs, supabase }) => {
            ...
            // Fetch logs, KG, and ac_assets from Supabase
            ...
            // ac_assets fetch removed — next_service now stored in ac_maintenance_logs
  ...
  const finalACAssets = useMemo(() => {
    ...
    return acAssets.map(modelAsset => {
      ...
      return {
        ...
        nextService: sortedSelfLogs.find((l: any) => l.next_service)?.next_service || ''
      }
    })
  ...
  const renderRightPanel = () => {
    const commonProps = {
      ...
      setShowDashboard, setAddLogAsset
    };
    switch (activeMode) {
  ...
  return (
    <div className="relative h-screen w-screen bg-sky-50 overflow-hidden font-sans select-none flex flex-row-reverse text-slate-900 p-[10px] gap-[10px]">
      {/* Add Log Modal — rendered at root level so it's never clipped by sidebar */}
      {addLogAsset && (
        <AddLogModal
          assetId={addLogAsset.id}
          assetDbId={(addLogAsset as any).dbId}
          roomCode={addLogAsset.id.split('-')[1] ? `rm-${addLogAsset.id.split('-')[1]}` : 'rm-101'}
          category="AC"
          onClose={() => setAddLogAsset(null)}
          onSuccess={() => { setAddLogAsset(null); window.dispatchEvent(new CustomEvent('refresh-bim-data')); }}
        />
      )}
