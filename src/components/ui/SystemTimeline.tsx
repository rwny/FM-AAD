    ...
    <div className="flex flex-col w-full gap-1 relative">
      {showLabels && (
        <div className="flex justify-between w-full text-[8px] font-black text-slate-400 uppercase tracking-tighter">
          <span>3Y</span>
          <span className="text-orange-600 italic">Today</span>
        </div>
      )}

      <div className="relative h-5 flex items-center" style={{ width }}>
        {/* Base Track */}
        <div className="absolute w-full h-1 bg-slate-100 dark:bg-zinc-800 rounded-full" />

        {/* Active Age Line */}
        {installMs >= startWindowMs && (
          <div
            className="absolute h-1 bg-zinc-800 dark:bg-zinc-200 rounded-full dark:shadow-[0_0_8px_rgba(255,255,255,0.3)]"
            style={{
        ...
        {installMs >= startWindowMs && (
          <div
            className="absolute w-2 h-2 bg-emerald-500 rotate-45 z-20 border border-white dark:border-zinc-800 shadow-sm"
        ...
        {markers.map((m, i) => (
          <div
            key={i}
            className={`absolute w-3 h-3 rounded-full border-2 border-white dark:border-zinc-800 shadow-sm z-30 cursor-pointer hover:scale-150 transition-transform ${
        ...
        {/* Today Indicator */}
        <div className="absolute right-0 w-0.5 h-3 bg-orange-400 dark:bg-orange-600 z-10 rounded" />
      </div>

      {tooltip && (
        <div
          className="fixed z-[200] bg-zinc-900 dark:bg-black text-white rounded-xl px-3.5 py-2.5 shadow-xl pointer-events-none text-sm leading-tight border border-white/10"
        ...
        >
          ...
          <div className="text-slate-400 dark:text-zinc-500 mt-0.5">{tooltip.date} Â· age {tooltip.age}mo</div>
          {tooltip.issue && <div className="text-slate-300 dark:text-zinc-400 mt-0.5">{tooltip.issue}</div>}
        </div>
      )}

      {installMs < startWindowMs && showLabels && (
        <div className="text-[7px] font-black text-slate-300 dark:text-zinc-600 uppercase tracking-tighter">
