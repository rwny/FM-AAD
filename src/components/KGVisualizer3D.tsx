...
export function KGVisualizer3D() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (fgRef.current) {
        ...
        const targetId = highlightLevel || hoverNodeId;
        if (targetId) {
  ...
  }, [isRotating, highlightLevel, hoverNodeId]);
  ...
  const triggerNodeHover = (node: any) => {
      ...
      if (hoverNodeId) {
        ...
        fadeTimeoutRef.current = setTimeout(() => {
          ...
          const pushInterval = setInterval(() => {
            if (count < 15) { setTerminalLogs(prev => [...prev, ""].slice(-25)); count++; }
            else { clearInterval(pushInterval); setTerminalLogs([]); }
          }, 50);
        }, 2000);
      }
      return;
    ...
    if (fadeTimeoutRef.current) clearTimeout(fadeTimeoutRef.current);
    const nodeLogs = [`[TARGET] ACQUISITION: ${node.name.toUpperCase()}`, `[IDENT] TYPE: ${node.type.toUpperCase()}`, `[LEVEL] DEPTH: ${node.level}`, `------------------------------------------`];
  ...
  const toggleRotation = () => {
    ...
    setTerminalLogs(prev => [...prev, `[SYSTEM] ${statusMsg}`].slice(-25));
  };
  ...
  const triggerHighlight = (level: string) => {
    ...
    const logsToStream = [
      `[SYSTEM] INITIATING DEEP SCAN: L${level}`, `[PROCESS] LOCATING ALL HIERARCHY NODES...`, `------------------------------------------`,
      ...levelNodes.map(n => `DATA_LINK: ${n.name.toUpperCase()} [${n.type.toUpperCase()}]`),
      `------------------------------------------`, `[INFO] ${levelNodes.length} NODES IDENTIFIED IN SCOPE.`, `[SUCCESS] SCAN_LEVEL_0${level}_COMPLETE`
    ];
    ...
    streamIntervalRef.current = setInterval(() => {
      if (currentIdx < logsToStream.length) {
        ...
        // PUSH OUT Animation (Enter key style)
        fadeTimeoutRef.current = setTimeout(() => {
          let pushCount = 0;
          const pushOutInterval = setInterval(() => {
            if (pushCount < 25) {
              setTerminalLogs(prev => [...prev, ""].slice(-25)); // Append empty lines to push content up
              pushCount++;
            } else {
              clearInterval(pushOutInterval);
              setTerminalLogs([]);
  ...
  return (
    <div className="absolute inset-0 bg-[#010409] overflow-hidden font-mono">
      <div className="absolute top-8 left-24 z-10 text-white pointer-events-none flex flex-col gap-0.5">
        <h2 className="text-3xl font-black text-white tracking-tighter drop-shadow-[0_0_15px_rgba(255,255,255,0.2)] font-mono">AR15 ASSET TOPOLOGY</h2>
        <p className="text-white text-[10px] font-black uppercase tracking-[0.4em] opacity-60 mb-4 font-mono">Hierarchical Infrastructure Intelligence</p>
      ...
      <div className="absolute top-8 right-8 z-20 flex gap-2">
        <button onClick={toggleRotation} className={`p-3 rounded-xl border backdrop-blur-xl transition-all shadow-2xl flex items-center gap-2 group ${isRotating ? 'bg-[#00f2ff]/20 border-[#00f2ff]/50 text-[#00f2ff]' : 'bg-black/40 border-white/10 text-white/40 hover:text-white hover:border-white/20'}`}>
      ...
      <ForceGraph3D
        ...
        nodeColor={(node: any) => (highlightLevel && node.level === highlightLevel) || (hoverNodeId === node.id) ? '#ffffff' : node.color}
        nodeRelSize={1.5} nodeResolution={24} onNodeHover={triggerNodeHover} nodeLabel={(node: any) => `${node.name} (${node.type})`}
        nodeVal={(node: any) => (highlightLevel && node.level === highlightLevel) || (hoverNodeId === node.id) ? node.val * 3 : node.val}
        ...
        linkDirectionalParticleColor={(link: any) => {
          ...
          if ((highlightLevel && sourceNode?.level === highlightLevel) || (hoverNodeId === sourceNode?.id)) return '#ffffff';
        ...
        nodeThreeObject={(node: any) => {
          ...
          const isTargeted = (highlightLevel && node.level === highlightLevel) || (hoverNodeId === node.id);
          if (isTargeted) {
